...
const KEY_TABLE_OPTIONS = 'table-fundraising-category-options';
...
export default {
  ...
  created() {
    console.log('***** Fundraising Category created() *****');
    this.fetchList();
  },
  methods: {
    ...
    doSaving() {
      ...
      axios.post('/api/fundraising-category', formData, config)
        .then((response) => {
    ...
    fetchList() {
      ...
      axios.get('/api/fundraising-category', {
        params: {}
      })
        .then((response) => {
    ...
    doDeleting(id) {
      ...
      axios.post('/api/fundraising-category', {
        id,
        _method: 'delete',
      })
        .then((response) => {
